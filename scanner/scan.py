...
def parse_results(trivy_path: str, checkov_path: str):
    ...
    with open("deduped_results.json", "w") as f: f.write(json.dumps(deduplicate_results(trivy_results, checkov_results)))
    

def deduplicate_results(trivy_results: dict, checkov_results: dict) -> dict:
    deduped_issues = dict() # dictionary key (which must be unique) is a tuple representing the resource and the normalized key
    ...
    for finding in checkov_results["results"]["failed_checks"]:
        ...
        deduped_issues[key] = (map_to_owasp(key), "checkov", finding["check_id"], None, finding["severity"], finding["file_line_range"][0], finding["file_line_range"][1])
    ...
    for target in trivy_results["Results"]:
        if target["MisconfSummary"]["Failures"] > 0:
            for finding in target["Misconfigurations"]:
                ...
                deduped_issues[key] = (map_to_owasp(key),"trivy", finding["ID"],finding["Resolution"], finding["Severity"], finding["CauseMetadata"]["StartLine"], finding["CauseMetadata"]["EndLine"])
